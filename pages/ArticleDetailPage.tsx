...
// ─── Konstanta domain — satu-satunya tempat yang perlu diubah jika domain bergeser
const SITE_URL  = 'https://beritakini.online';
const SITE_NAME = 'Berita Kini';

// ─── Helper: buat URL artikel lengkap
const articleUrl = (slug: string) => `${SITE_URL}/artikel/${slug}`;

// ─── Helper: potong teks untuk description (tanpa memotong kata)
const makeExcerpt = (text: string, maxLen = 155): string => {
  const plain = text.replace(/\n/g, ' ').trim();
  if (plain.length <= maxLen) return plain;
  return plain.substring(0, plain.lastIndexOf(' ', maxLen)) + '…';
};
...
const ArticleDetailPage: React.FC = () => {
  ...
  // useEffect berjalan setiap kali artikel berubah (navigasi ke slug baru).
  // window.updatePageMeta() didefinisikan di index.html.
  //
  // Ini memastikan:
  // 1. Googlebot (yang menjalankan JS) membaca meta yang benar
  // 2. WhatsApp/Telegram mengambil OG image dari URL absolut yang benar
  // 3. Tidak ada lagi "isdnews.com" di share links
  // ─────────────────────────────────────────────────────────────────────────
  useEffect(() => {
    if (!article) {
      // Reset ke default jika artikel tidak ditemukan
      if (typeof window.updatePageMeta === 'function') {
    ...
    const desc = makeExcerpt(article.content);
    ...
    if (typeof window.updatePageMeta === 'function') {
      window.updatePageMeta({
        ...
        description:   desc,
        url,
        // imageUrl dari picsum.photos sudah bersifat absolut — langsung dipakai
        // sebagai og:image. WhatsApp/Telegram akan mengambil gambar ini.
        ...
        schema: {
          ...
          description:    desc,
          articleSection: CATEGORIES.find(c => c.id === article.categoryId)?.name || '',
  ...
  // Reset meta ke default saat komponen di-unmount
  useEffect(() => {
    return () => {
      if (typeof window.updatePageMeta === 'function') {
        window.updatePageMeta({
          title:       `${SITE_NAME} — Portal Berita Terpercaya Indonesia`,
          description: 'Menyajikan berita terkini, analisis mendalam, dan jurnalisme berkualitas untuk masyarakat Indonesia.',
          url:         '/',
          type:        'website',
        });
      }
  ...
  // ─── URL share yang benar (pakai SITE_URL, bukan isdnews.com) ───
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-0">
          ...
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-4 pb-6 border-b border-ink-100">
              ...
              {/* ─── SHARE BUTTONS — pakai shareUrl (beritakini.online) ─── */}
              <div className="flex items-center gap-2 ml-auto">
                ...
                <button
                  ...
                  id="copy-link-btn"
                  className="w-8 h-8 rounded-full bg-ink-700 text-white flex items-center justify-center hover:opacity-80 transition-opacity text-xs"
            ...
            <figure className="mb-8 rounded-2xl overflow-hidden shadow-lg">
              ...
              <figcaption className="bg-ink-900 text-ink-400 text-xs px-4 py-2.5 text-center">
                {article.imageCaption
                  ? article.imageCaption
                  : `Foto: ${article.authorName} / ${SITE_NAME}`}
              </figcaption>
            </figure>
              ...
              <div className="mt-8 pt-6 border-t border-ink-100 bg-ink-50 -mx-8 -mb-8 px-8 pb-8 rounded-b-2xl">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-ink-500">Jurnalis &amp; Kontributor {SITE_NAME}</p>
                  </div>
                </div>
              </div>
            </div>
...
// ─── Type augmentation: supaya TypeScript tidak komplain soal window.updatePageMeta ───
declare global {
  interface Window {
    updatePageMeta?: (meta: {
      title?: string;

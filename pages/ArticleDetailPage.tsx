...
const ArticleDetailPage: React.FC = () => {
  ...
  if (!article) {
    return (
      <div className="max-w-2xl mx-auto py-24 px-4 text-center">
        ...
        <h2 className="font-serif text-2xl font-bold text-ink-900 mb-2">Artikel tidak ditemukan</h2>
  ...
  const dateStr = publishDate.toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
      ...
      <div className="bg-white border-b border-ink-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-0">
          ...
          <nav className="flex items-center gap-2 text-xs text-ink-400 mb-6">
            ...
            {category && (
              <>
                <Link to={`/kategori/${category.id}`} className="hover:text-brand-500 transition-colors">{category.name}</Link>
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
              </>
            )}
            <span className="text-ink-500 line-clamp-1">{article.title.substring(0, 40)}...</span>
          </nav>
          ...
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-4 pb-6 border-b border-ink-100">
              ...
              <div className="flex items-center gap-1.5 text-ink-400">
                ...
                <span className="text-xs">{dateStr} · {timeStr} WIB</span>
              </div>
              {/* Share */}
              <div className="flex items-center gap-2 ml-auto">
                <span className="text-xs text-ink-400 font-medium">Bagikan:</span>
                <button className="w-8 h-8 rounded-full bg-[#1DA1F2] text-white flex items-center justify-center hover:opacity-80 transition-opacity text-xs font-bold">X</button>
                <button className="w-8 h-8 rounded-full bg-[#1877F2] text-white flex items-center justify-center hover:opacity-80 transition-opacity text-xs font-bold">f</button>
                <button className="w-8 h-8 rounded-full bg-[#25D366] text-white flex items-center justify-center hover:opacity-80 transition-opacity text-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="bg-white rounded-2xl p-6 sm:p-8 border border-ink-100">
              <div className="article-body text-ink-700 text-lg leading-relaxed">
                {article.content.split('\n\n').map((para, idx) => (
                  para.trim() ? <p key={idx} className="mb-5">{para}</p> : null
                ))}
              </div>
              ...
              {article.tags && article.tags.length > 0 && (
                <div className="mt-8 pt-6 border-t border-ink-100 flex flex-wrap items-center gap-2">
                  <svg className="w-4 h-4 text-ink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
                  {article.tags.map(tag => (
                    <span key={tag} className="px-3 py-1 bg-ink-100 text-ink-600 text-xs rounded-full font-medium hover:bg-brand-50 hover:text-brand-600 transition-colors cursor-pointer">
                      #{tag}
                    </span>
                  ))}
                </div>
            ...
            {relatedArticles.length > 0 && (
              <section className="mt-10">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <h3 className="font-serif text-xl font-bold text-ink-900">Berita Terkait</h3>
                </div>
          ...
          <aside className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-ink-100 p-5 sticky top-36">
              ...
              <Link to="/" className="mt-4 w-full flex items-center justify-center gap-1.5 text-sm font-medium text-brand-500 hover:text-brand-700 transition-colors">
                Lihat Semua Berita

...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulated async auth with timeout to prevent brute-force UX
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-ink-50 flex" role="main">
      {/* Left Panel — Branding */}
      <div className="hidden lg:flex lg:w-1/2 bg-ink-900 flex-col justify-between p-12 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-brand-900/40 via-ink-900 to-ink-900 pointer-events-none" aria-hidden="true"></div>
        <div className="absolute -bottom-20 -right-20 w-96 h-96 rounded-full bg-brand-500/10 blur-3xl pointer-events-none" aria-hidden="true"></div>
        <div className="absolute top-20 -left-20 w-80 h-80 rounded-full bg-brand-500/5 blur-3xl pointer-events-none" aria-hidden="true"></div>

        {/* Logo */}
        <div className="relative z-10">
          <Link to="/" className="inline-flex items-baseline gap-1.5 group" aria-label="ISD NEWS - Kembali ke Beranda">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12">
        <div className="w-full max-w-md">
          ...
          <div className="lg:hidden text-center mb-8">
            <Link to="/" className="inline-flex items-baseline gap-1.5" aria-label="ISD NEWS - Beranda">
          ...
          <div className="mb-8">
            <h1 className="font-serif text-3xl font-bold text-ink-900 mb-2">Masuk ke Akun</h1>
          ...
          >
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <a href="#" className="text-xs text-brand-500 hover:text-brand-700 transition-colors">
                  Lupa password?
                </a>
              </div>
              <div className="relative">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-brand-500 hover:bg-brand-600 disabled:opacity-60 disabled:cursor-not-allowed text-white font-semibold text-sm rounded-lg transition-colors"
              aria-busy={loading}
            >
              {loading ? (
                <>

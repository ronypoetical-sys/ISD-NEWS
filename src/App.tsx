    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-left">
          ...
          <tbody>
            {myArticles.length === 0 && <tr><td colSpan={2} className="p-8 text-center text-gray-400 italic">Belum ada berita yang Anda tulis.</td></tr>}
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 py-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
        <div>
          <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Judul Berita</label>
          <input name="title" required className="w-full border-b-2 border-gray-100 p-2 text-2xl font-bold outline-none focus:border-red-500" placeholder="Tulis judul yang menarik..." />
        </div>
        <div>
          <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Link Foto Cover (URL)</label>
          <input name="image" required className="w-full border-b-2 border-gray-100 p-2 outline-none focus:border-red-500" placeholder="https://..." />
        </div>
        <div>
          <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Isi Berita</label>
          <textarea name="content" required rows={15} className="w-full border-2 border-gray-50 p-4 rounded-xl outline-none focus:border-red-500 leading-relaxed" placeholder="Tulis berita lengkap Anda di sini..."></textarea>
        </div>
        <button disabled={loading} className="w-full bg-red-600 text-white py-4 rounded-2xl font-bold text-lg hover:bg-red-700 transition-all shadow-lg shadow-red-200 disabled:bg-gray-300">
          {loading ? "Sedang Mengirim..." : "Publikasikan Berita"}
        </button>
      </form>
    </div>
  );

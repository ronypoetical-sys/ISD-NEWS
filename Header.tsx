...
const Header: React.FC = () => {
  ...
  /*
    Duplikasi 3x agar animasi -33.333% loop tanpa lompatan.
    PENTING: jangan kurang dari 3x duplikasi.
  */
  const tickerItems = [
    ...latestHeadlines,
    ...latestHeadlines,
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-shadow duration-300 ${isScrolled ? 'header-scrolled' : ''}`}
      role="banner"
    >
      {/* ── Top Bar ── */}
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  ...
                  {user.role === UserRole.USER && (
                    <Link to="/dashboard" className="text-ink-300 hover:text-white transition-colors">
                      Dashboard
                    </Link>
                  )}
                  <button
      ...
      {/* ══════════════════════════════════════════════════════
          NEWS TICKER — Mobile & Desktop Compatible
          ══════════════════════════════════════════════════════
          Semua style animasi didefinisikan di index.html <style>
          dengan class .ticker-track dan @keyframes tickerMove.
          Header.tsx HANYA menggunakan class name — TIDAK ada
          inline animation style — ini kunci agar Android tidak
          konflik antara CSS class dan inline style.
          ══════════════════════════════════════════════════════ */}
      {latestHeadlines.length > 0 && (
        ...
        >
          ...
          >
            {/* Label TERKINI — tidak ikut bergerak */}
            <div
              aria-hidden="true"
              style={{
                flexShrink: 0,
            ...
            >
              <span className="breaking-dot" style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'white', display: 'inline-block', flexShrink: 0 }} />
              TERKINI
            </div>

            {/*
              Wrapper clips konten yang overflow.
              flex:1 agar mengambil sisa ruang setelah badge "TERKINI".
            */}
            <div className="ticker-track-outer" style={{ flex: 1, height: '100%' }}>
              {/*
                .ticker-track → CSS class dari index.html yang berisi
                @keyframes tickerMove (translateX 0 → -33.333%).
                TIDAK ada animasi inline di sini sama sekali.
              */}
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="flex-shrink-0 group" aria-label="ISD NEWS - Beranda">
              <div className="flex items-baseline gap-1">
                <span className="font-serif text-3xl text-brand-500 group-hover:text-brand-600 transition-colors leading-none tracking-tight">
                  ISD
                </span>
                <span className="text-sm font-bold text-ink-800 tracking-[0.3em] uppercase leading-none">
                  NEWS
                </span>
              </div>
              <div className="text-[9px] text-ink-400 tracking-widest uppercase font-light mt-0.5">
                  ...
                  className={({ isActive }) =>
                    `px-3 py-2 text-sm font-medium rounded transition-colors ${
                      isActive
                        ...
                        : 'text-ink-700 hover:text-brand-500 hover:bg-ink-50'
                    }`
                  }
                >
                  {item.label}
              ...
              <div className="relative">
                {showSearch ? (
                  <form onSubmit={handleSearch} className="flex items-center" role="search">
                    ...
                    <button
                      type="submit"
                      className="px-3 py-1.5 bg-brand-500 text-white rounded-r-md hover:bg-brand-600 transition-colors"
                      aria-label="Cari"
                    >
                    ...
                    <button
                      ...
                      className="ml-1 text-ink-400 hover:text-ink-700 p-1"
      ...
      <nav
        className="bg-ink-900 hidden lg:block border-b border-ink-800"
        aria-label="Navigasi kategori berita"
      >
      ...
      {isMenuOpen && (
        <div
          id="mobile-menu"
          className="lg:hidden bg-white border-b border-ink-100 shadow-xl animate-slide-up"
          role="navigation"
          ...
          {user && (
            <div className="border-t border-ink-100 px-4 py-3 flex items-center justify-between">
              ...
              <button
                ...
                className="text-sm text-brand-500 font-medium hover:text-brand-700"
              >
                Keluar
              </button>
            </div>

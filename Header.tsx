...
const Header: React.FC = () => {
  ...
  // Duplikasi 3x → animasi -33.333% = loop sempurna tanpa jeda
  const tickerItems = [
    ...latestHeadlines,
    ...latestHeadlines,
    ...latestHeadlines,
  ...
  return (
    ...
    >
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  ...
                  {user.role === UserRole.ADMIN && (
                    <Link to="/admin" className="bg-brand-500 text-white px-2 py-0.5 rounded text-xs font-medium hover:bg-brand-600 transition-colors">Admin Panel</Link>
                  )}
                  {user.role === UserRole.USER && (
                    <Link to="/dashboard" className="text-ink-300 hover:text-white transition-colors">Dashboard</Link>
                  )}
                  <button onClick={logout} className="text-ink-400 hover:text-white transition-colors" aria-label="Keluar dari akun">Keluar</button>
                </div>
              ) : (
                <div className="flex items-center gap-3">
      ...
      {/* ============================================================
          NEWS TICKER
          PENTING: Tidak ada satupun style animasi di sini (JSX).
          Semua animasi 100% dihandle oleh CSS class di index.html:
            .news-ticker-track  →  animation: newsTickerScroll 60s
            .news-ticker-outer  →  overflow: hidden
            .news-ticker-item   →  padding & font
          Ini adalah satu-satunya cara agar tidak ada konflik
          antara CSS class vs inline style di Android WebView.
          ============================================================ */}
      {latestHeadlines.length > 0 && (
        ...
        >
          ...
          >
            ...
            >
              <span
                className="breaking-dot"
                style={{ width: 6, height: 6, borderRadius: '50%', background: '#fff', display: 'inline-block', flexShrink: 0 }}
              />
              TERKINI
            </div>

            {/* Ticker outer — overflow hidden, mengambil sisa lebar */}
            <div className="news-ticker-outer">
              {/*
                .news-ticker-track mendapat animasi dari CSS global di index.html.
                JANGAN tambahkan style animasi apapun di sini.
              */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden lg:flex items-center gap-1" aria-label="Navigasi utama">
              {mainNavItems.map(item => (
                <NavLink key={item.to} to={item.to} end={item.exact}
                  className={({ isActive }) =>
                    `px-3 py-2 text-sm font-medium rounded transition-colors ${isActive ? 'text-brand-500 bg-brand-50' : 'text-ink-700 hover:text-brand-500 hover:bg-ink-50'}`
                  }>
                  {item.label}
                </NavLink>
              ))}
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                {showSearch ? (
                  <form onSubmit={handleSearch} className="flex items-center" role="search">
                    ...
                    <input ref={searchInputRef} id="header-search" type="search" value={searchQuery}
                      onChange={e => setSearchQuery(e.target.value)} placeholder="Cari berita..."
              ...
              {!user && (
                <Link to="/login" className="hidden sm:inline-flex items-center px-4 py-2 bg-brand-500 text-white text-sm font-medium rounded-md hover:bg-brand-600 transition-colors">Masuk</Link>
              )}
        ...
        <div id="mobile-menu" className="lg:hidden bg-white border-b border-ink-100 shadow-xl animate-slide-up" role="navigation" aria-label="Menu mobile">
          ...
          {user && (
            <div className="border-t border-ink-100 px-4 py-3 flex items-center justify-between">
              ...
              <button onClick={() => { logout(); closeMenu(); }} className="text-sm text-brand-500 font-medium hover:text-brand-700">Keluar</button>
            </div>
          )}
        </div>
      )}
